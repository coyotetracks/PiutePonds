...
\*--------------------------------------------------------------------------------------------*/
function validateFields($category, $fields, $rules)
{
  $errors = array();
  ...
  //var_dump_jeff('errors in validate', $errors);

  $categoryErrorsKey = generateCategoryErrorsKey($category);
  //var_dump_jeff('categoryErrorsKey', $categoryErrorsKey);

  $allErrors = $_SESSION['errors'];
  //var_dump_jeff('allErrors', $allErrors);

  $categoryErrors = $allErrors[$categoryErrorsKey];
  //var_dump_jeff('categoryErrors', $categoryErrors);
  //exit;

  if(!isset($categoryErrors)) {
    //echo 'categoryErrors is not set<BR>';
  	$categoryErrors = array();
  	foreach($errors as $oneError) {
  		$categoryErrors[] = $oneError;
  	}
    //var_dump_jeff('categoryErrors with errors', $categoryErrors);
  } else {
  	foreach($errors as $oneError) {
  		$categoryErrors[] = $oneError;
  	}
  	$allErrors[$category] = $categoryErrors;
    //var_dump_jeff('existing categoryErrors with new errors', $categoryErrors);
  }

  // Clear the old errors.
  //$_SESSION['errors'] = array();

  $_SESSION[$categoryErrorsKey] = $categoryErrors;

  // Set the new errors.
  //$_SESSION['errors'] = $allErrors;
  //var_dump_jeff('session', $_SESSION);
  //exit;
...
function generateCategoryErrorsKey($categoryName) {
	return 'errors_'.$categoryName;
}

function getCategoryErrors($category) {
	$categoryErrorsKey = generateCategoryErrorsKey($category);
	//var_dump_jeff('allErrors', $allErrors);
    if(isset($_SESSION[$categoryErrorsKey])) {
	$categoryErrors = $_SESSION[$categoryErrorsKey];
    } else {
        $categoryErrors = array();
    }
	//var_dump_jeff('categoryErrors', $categoryErrors);
...
////var_dump($sessionErrors);
//	$sessionErrors[] = $newError;
//	$_SESSION['errors'] = $sessionErrors;
//}

function addError($category, $newError) {
	$categoryErrorsKey = generateCategoryErrorsKey($category);
	$categoryErrors = $_SESSION[$categoryErrorsKey];
//var_dump_jeff('categoryErrors', $categoryErrors);
	$categoryErrors[] = $newError;
	$_SESSION[$categoryErrorsKey] = $categoryErrors;
}
...
function clearCategoryErrors($category) {
	$sessionErrors = $_SESSION['errors'];
	$sessionErrors[$category] = array();
	$_SESSION['errors'] = $sessionErrors;
}

function getErrorMessages($category) {
	$formattedErrorMessages = "";

	$errors = getCategoryErrors($category);
	//var_dump_jeff('getCategoryErrors', $errors);
	//exit;

    if (!empty($errors)) {
    	foreach ($errors as $error) {
			$formattedErrorMessages = $formattedErrorMessages.$error."<BR>";
    	}
    	clearCategoryErrors($category);
    }
	return $formattedErrorMessages;
}

function echoErrorMessages($category) {
	echo getErrorMessages($category);
}

function var_dump_jeff($label, $object) {
