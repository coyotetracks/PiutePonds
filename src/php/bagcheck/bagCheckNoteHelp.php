...
function generateReadNoteSql($noteId) {
	$noteSql = "";
	$noteSql .= "SELECT * FROM bc_note WHERE ";
	$noteSql .= formatSqlSetNumericValue('note_id', $noteId, false);
	return $noteSql;
}
...
function readNote($noteId) {
	...
	$readNoteSql = generateReadNoteSql($noteId);
	
	$result = mysql_query($readNoteSql, $dbLink);
	if (!$result) {
		throw new Exception('Select of Note Failed: ' . $readNoteSql);
	} else {
		$row = mysql_fetch_assoc($result);
		$noteContent = $row['bc_note'];
	}
...
// function setBagCheckInfoInSession($bagCheckBlindList) {
// 	$_SESSION["bag_check_info"] = $bagCheckBlindList;
// }
...
// // 	$oldDateLabel = generateOldIdAtDay("DATE");
// // 	$oldDate = $_POST[$oldDateLabel];
// // 	$newDateLabel = generateNewIdAtDay("DATE");
// // 	//$newDateLabel = "huntDateAnchor";
// // 	$newDate = $_POST[$newDateLabel];
// // 	$newDate = unformatDate($newDate);
// // 	if($oldDate != $newDate) {
// // 		$bagCheckInfo->setDate($newDate);
// // 	}
// }
